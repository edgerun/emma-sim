...
from simulation.processes import DistanceBasedBrokerScalerProcess
...
class EmmaScenario(Scenario):
    ...
    def scenario_process(self):
        ...
        context = NetworkContext(self.broker_procs, self.client_procs, self.topology)
        self.env.run(DistanceBasedBrokerScalerProcess(self.env, context, 0.2, 0.8))
        for _ in range(5):
            self.create_cloud_broker('eu-central')
            self.create_cloud_broker('eu-west')
            self.create_cloud_broker('us-east')
        yield self.sleep()
